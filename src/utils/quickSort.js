export function quickSort(arr, key, type) {
    ...
    if (type === 2) { //降序
        ...
        return quickSort(left, key, 2).concat([_m], quickSort(right, key, 2))
...
export function quickSortTime(arr, key, type) { //按时间快速排序
    if (arr.length <= 1) { return arr }
    let left = []
    let right = []
    let _index = Math.floor(arr.length / 2)
    ...
    if (type === 2) { //降序
        ...
        return quickSortTime(left, key,2).concat([_m], quickSortTime(right, key,2))
