...
import "../../assets/scss/pc/exchange/exchange_submit.css";
import { getTradeRecordsList } from "../../redux/trade.redux";

import MediaQuery from "react-responsive";

@withRouter
@connect(state => ({ ...state.trade, ...state.sdetails, ...state.gconfig, ...state.user, ...state.assets, ...state.lang,...state.index }),
{
  getPropetyInfo,
  saveOrderPost,
  getOpenOrders,
...
class PayBix extends Component {
      ...
      this.props.saveOrderPost(data).then((response)=>{
        ...
        if (response.data.errno=="OK") {
          ...
          if (this.props.bbx_show_all) {
            ...
            this.props.getTradeRecordsList(this.state.coinPair);
          }
        }else {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.stocks && nextProps.stocks.length>0 && nextProps.bbx_ticker_info) {
      let stocks = this.getStocksList(nextProps.stocks);
      let ticker = nextProps.bbx_ticker_info;
      let volUnit = getVolUnit(stocks,coinPair);
      let priceUnit = getPriceUnit(stocks,coinPair);
      let minTradeAmount = this.getMinTradeAmount(stocks, coinPair);
      let tipLimit = this.getTipLimit(stocks, coinPair);
      if (this.mounted) {
        this.setState({
          ticker: ticker,
        ...
        //用这部分#########
        //判断是否切换币值对 和sell_price和buy_price是否有值
        if (this.props.current_coin_pair !== nextProps.current_coin_pair || !this.state.form.buy_price.value || !this.state.form.sell_price.value) {
          if(this.mounted) {
            this.setState({
              buy_silder: 0,  //置0买选择条
    ...
    //费率列表（新）
    if (token && nextProps.stocks) {
      let coin_fee_configs = this.getCoinFeeList(nextProps.stocks);
      if (this.mounted) {
        this.setState({
          coin_fee_configs: coin_fee_configs
        })
      }
    ...
    //用户资产
    if (nextProps.bbx_assets_list) {
      if(this.mounted) {
        this.setState({
          user_assets: nextProps.bbx_assets_list
        })
      }
    }
    ...
    // 判断是否切换了币值对，是否支持市价单
    if (nextProps.stocks && nextProps.current_coin_pair !== this.props.current_coin_pair && nextProps.clist.spot_coins) {
      let stocks = this.getStocksList(nextProps.stocks);
      this.setIssupportMarket(stocks, nextProps.current_coin_pair);
      //console.log("nextProps.spot_details.stock_code####", nextProps.spot_details.stock_code);
      let coinPair = nextProps.current_coin_pair;
  ...
  render() {
    ...
    const { currentTab, coinPair } = this.state;
    ...
    let langCode = "", search="", loginSearch="";
    ...
    loginSearch = search + "&" + `path=/exchange${ window.location.search }`;
                  ...
                  {!!token ? (
                    <button className="buy-btn" disabled={!this.state.canSubmitBuy} onClick={() => this.submitOrder(1)}>{intl.get("assets_order_tab_buy")} {coin1_name}</button>
                      ...
                      <div className="btn-style">
                        <Link to={{ pathname: "/login", search: `${loginSearch}` }}>{intl.get("exchange_btn_login")}</Link> {intl.get("exchange_btn_or")} <Link to={{ pathname: "/register", search: `${loginSearch}` }}>{intl.get("exchange_btn_register")}</Link> {intl.get("exchange_btn_to_trade")}
                      </div>
                    )}
                </div>
              </div>
